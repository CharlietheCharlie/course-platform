...
export function revalidateUserLessonCompleteCache({ id, lessonId }: {
  id: string;
  lessonId: string;
}) {
    revalidateTag(getUserLessonCompleteGlobalTag());
    revalidateTag(getUserLessonCompleteIdTag({lessonId, userId: id}));
    revalidateTag(getUserLessonCompleteUserTag(id));
}
